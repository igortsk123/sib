...
- **DocON** — публичный БРЕНД продукта (домен docon.pro, бот doconpro_bot, лендинг, кампании Директа). **sib** — внутреннее кодовое имя проекта (от первого клиента — «Клиника Сибирская»); в публичных текстах не используется (владелец, 26.07).
