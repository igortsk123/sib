...
вердикт); **чат по правилам** (`chatAboutCoverage`, digest без ПДн). LLM — только если правила
молчат. Черновик запроса ГП reply-in-thread (`guarantee-request.ts`, .eml, БЕЗ отправки).
...
`/patients`+карточка (вопрос-ответ, теги, чат, запрос ГП). 13 документов, ~890 правил
