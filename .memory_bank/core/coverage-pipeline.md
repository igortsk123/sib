...
`poll_program_docs.py` (sha) → `extract_doc_texts.py` → `carry_coverage_rules.py` (перенос;
изменившийся пункт → `needs_review`) → `registry_watch.py` (D33: реестры сайтов СК, редакции
новее нашей скачиваются и ждут экстракции). Онбординг СК/клиники — `../guides/onboarding.md`.

## Ручной шаг — экстракция (агент, без внешних LLM)
Промпты A–E × Ф1–Ф7 + чек-лист: `guides/coverage-extraction-prompt.md`; прогоны —
...
`answer-core.ts` — гейты: прикреплён? → действующее ГП → правила → лимит; **catch-all**: услуга
не названа + сноска «непредусмотренное не покрывается» → чёткий НЕТ с пунктом; **уточнения-теги**
из conditional-правил; **чат по правилам** — главный UI карточки (D41): история в БД, общая на пациента,
автор виден; правила целиком + хвост истории в LLM; без LLM — детерминированный ответ; ОТКРЕПЛЁННЫЙ пациент → мгновенный ответ «откреплён с даты» БЕЗ вызова LLM;
GENERIC_WORDS против ложных совпадений. Черновик запроса ГП reply-in-thread (.eml, БЕЗ отправки).
