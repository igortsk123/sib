...
- **Inngest** — durable-задачи (IMAP, вложения, OCR, LLM, распаковка), НЕ в request-пути.
  **Статус: каркас** — в коде только `ping`; прикладные шаги по срезам S1+.
...
## Пайплайн (дизайн §15 — реализуется срезами S1+)
