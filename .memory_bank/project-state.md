...
- ⏭ **Дальше:** IMAP-забор прод-пайплайна (роадмап S1 — тулзы pdftotext/striprtf/xlrd/olefile/libreoffice
  в Docker); правка полей в карточке; per-insurer механические шаблоны; vision на реальные сканы.
- ⏭ **SMTP-отправка** (Яндекс XOAUTH2, `smtp.yandex.ru`) — наружу, нужен рабочий OAuth-токен + проверка
  коннективности (outward-facing) → **с владельцем**. Пока письма логируются как pending (`notifyErrorFixed`).
