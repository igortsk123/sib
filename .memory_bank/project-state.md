...
- ⏭ **Очередь (следующее):**
  1. Выбор клиники у платформенного админа (контекст реестра, мультитенант-скоуп).
  2. Дозаполнение полей (повторный LLM-проход / vision на сканы), привязка записей к клинике.
