...
- ⏭ **Очередь (следующее):**
  1. DOC/бинарный .xls (libreoffice/catdoc на сервере) + ZIP AES (Зетта/Росгосстрах) + vision на сканы.
  2. Выбор клиники у платформенного админа (контекст реестра); привязка записей к клинике.
  3. IMAP-забор (роадмап S1) — отложен.
