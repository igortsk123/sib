...
- ✅ **АССИСТЕНТ ПОКРЫТИЯ ДМС — конвейер собран (2026-07-25, ADR D23–D27):** документы страховых →
  правила → карточка пациента. **13 документов, 370 правил, покрытие ~46% пациентов** (считается онлайн
  по прикреплениям; гарантийные письма не участвуют — в них услуги, а не программы).
...
- ⏭ **Дальше:** ответ врачу и регистратуре «можно ли сделать X пациенту за Y» (детерминированные гейты →
  mini-LLM только на неясное) и генератор запроса ГП; хвосты распознавания: vision на реальные сканы,
  .doc-экстрактор в раннер (ЭГ), L8-метки Альфа/Ингосстрах, кир/лат полисы Ингосстрах.
