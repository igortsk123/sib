...
# Очистка после деплоя — висячие образы (БЕЗ -a → чужие проекты не трогаем).
docker image prune -f >/dev/null 2>&1 || true
# Build-cache держим ТЁПЛЫМ, но ОГРАНИЧИВАЕМ (рос до 17GB → диск 100%, деплой падал «no space left»).
# keep-storage сохраняет свежий кэш для быстрого ребилда, лишнее удаляет. Фолбэк — полная чистка кэша.
docker builder prune -f --keep-storage 3GB >/dev/null 2>&1 || docker builder prune -af >/dev/null 2>&1 || true
