...
// Граница контуров (ADR D50) — одно правило на весь код: lib/server/demo-org.ts.
const scopeOrg = (orgId: string | null) => orgScope(sql`${guaranteeLetter.organizationId}`, orgId)
  ...
  const scope =
    orgId === "__none__"
      ...
      : orgId
        ...
        : scopeOrg(null) // админ «все клиники» — только рабочий контур
  const r = await db().select({ n: sql<number>`count(*)::int` }).from(guaranteeLetter)
